package com.ntduc.numberpicker
...
//            initializeFadingEdges()
    ...
    private fun tryComputeMaxWidth() {
        ...
        if (mMaxWidth != maxTextWidth) {
            mMaxWidth = max(maxTextWidth, mMinWidth)
            invalidate()
        }
    }
     ...
     * wrap around the [minValue] and [maxValue] values.
     */
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in selectorIndices.indices) {
            if (i == mWheelMiddleItemIndex) {
                ...
                mSelectorWheelPaint.typeface = ResourcesCompat.getFont(context, R.font.inter_extrabold)
            } else {
                mSelectorWheelPaint.textAlign = Paint.Align.values()[textAlign]
                val factorTextSize = if (mWheelMiddleItemIndex > i) (1 - (mWheelMiddleItemIndex - i) * 0.1) else (1 - (i - mWheelMiddleItemIndex) * 0.1)
                mSelectorWheelPaint.textSize = (mTextSize * factorTextSize).toFloat()
                mSelectorWheelPaint.color = mTextColor
                val factorTextColor = if (mWheelMiddleItemIndex > i) (1 - (mWheelMiddleItemIndex - i) * 0.2) else (1 - (i - mWheelMiddleItemIndex) * 0.2)
                mSelectorWheelPaint.alpha = (Color.alpha(mTextColor) * factorTextColor).roundToInt()
                mSelectorWheelPaint.isStrikeThruText = textStrikeThru
                mSelectorWheelPaint.isUnderlineText = textUnderline
                mSelectorWheelPaint.typeface = ResourcesCompat.getFont(context, R.font.inter_custom)
            }
...
//        }
    ...
    private fun setValueInternal(current: Int, notifyChange: Boolean) {
        ...
        if (mWrapSelectorWheel) {
            ...
            current = max(current, mMinValue)
            current = min(current, mMaxValue)
        }
        val previous = mValue
        mValue = current
    ...
    private fun updateInputTextView() {
         ...
         * number.
         */
        val text = if (displayedValues == null) formatNumber(mValue) else displayedValues!![mValue - mMinValue]
    ...
    internal inner class InputTextFilter : NumberKeyListener() {
        ...
        override fun filter(
            ...
            return if (displayedValues == null) {
                ...
                for (values in displayedValues!!) {
                    val valLowerCase = values.lowercase(Locale.getDefault())
                    if (valLowerCase.startsWith(str)) {
                        postSetSelectionCommand(result.length, values.length)
                        return values.subSequence(dstart, values.length)
                    }
                }
                ""
            }
    ...
    fun setWheelItemCount(count: Int) {
        ...
        mWheelItemCount = max(count, DEFAULT_WHEEL_ITEM_COUNT)
