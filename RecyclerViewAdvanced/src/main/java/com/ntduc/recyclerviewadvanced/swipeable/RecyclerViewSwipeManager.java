...
import com.ntduc.recyclerviewadvanced.adapter.ItemIdComposer;
import com.ntduc.recyclerviewadvanced.animator.SwipeDismissItemAnimator;
import com.ntduc.recyclerviewadvanced.swipeable.action.SwipeResultAction;
import com.ntduc.recyclerviewadvanced.swipeable.action.SwipeResultActionDefault;
import com.ntduc.recyclerviewadvanced.utils.CustomRecyclerViewUtils;
import com.ntduc.recyclerviewadvanced.utils.WrapperAdapterUtils;
...
public class RecyclerViewSwipeManager implements SwipeableItemConstants {
    ...
    public void attachRecyclerView(@NonNull RecyclerView rv) {
        ...
        mItemSlideAnimator.setImmediatelySetTranslationThreshold((int) (rv.getResources().getDisplayMetrics().density * SLIDE_ITEM_IMMEDIATELY_SET_TRANSLATION_THRESHOLD_DP + 0.5f));
    ...
    private void finishSwiping(int result) {
        ...
        switch (afterReaction) {
            ...
            case AFTER_SWIPE_REACTION_REMOVE_ITEM: {
                ...
                final RemovingItemDecorator decorator = new RemovingItemDecorator(mRecyclerView, swipingItem, result, removeAnimationDuration, moveAnimationDuration);
    ...
    public boolean performFakeSwipe(RecyclerView.ViewHolder holder, int result) {
        ...
        MotionEvent fakeMotionEvent = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, 0, 0, 0);
    ...
    /*package*/ void applySlideItem(RecyclerView.ViewHolder holder, int itemPosition, float prevAmount, float amount, boolean proportionalAmount, boolean horizontal, boolean shouldAnimate, boolean isSwiping) {
    ...
    private static int determineBackgroundType(float amount, boolean horizontal) {
        if (horizontal) {
            return (amount < 0) ? DRAWABLE_SWIPE_LEFT_BACKGROUND : DRAWABLE_SWIPE_RIGHT_BACKGROUND;
        } else {
            return (amount < 0) ? DRAWABLE_SWIPE_UP_BACKGROUND : DRAWABLE_SWIPE_DOWN_BACKGROUND;
        }
    }
